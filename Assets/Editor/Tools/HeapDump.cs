    ...
    public class HeapDump
    {
        ...
        public void DumpToXml()
        {
            ...
            using( writer = new StreamWriter( "heapdump.xml" ) )
            {
                ...
                foreach( var type in allTypes )
                {
                    ...
                    if(!SkipEmptyTypes)
                        writer.WriteLine( "  <type name=\"{0}\">", SecurityElement.Escape( type.Name ) );
                    if(type.IsEnum)
                    {
                    ...
                    else
                    {
                        foreach( var fieldInfo in type.GetFields( BindingFlags.Static | BindingFlags.Public | BindingFlags.NonPublic ) )
                        {
                            ...
                            {
                                writer.WriteLine( "Exception: " + ex.Message + " on " + fieldInfo.FieldType.Name + " " +
                                                  fieldInfo.Name );
                            }
                        }
                    }
                ...
                {
                    writer.WriteLine( "  <object type=\"{0}\" name=\"{1}\">", SecurityElement.Escape( mb.GetType().Name ), SecurityElement.Escape( mb.name ) );
                    var type = mb.GetType();
        ...
        public int GatherFromRootRecursively(object root, string depth)
        {
            ...
            foreach( var fieldInfo in fields )
            {
                ...
                {
                    writer.WriteLine( "Exception: " + ex.Message + " on " + fieldInfo.FieldType.Name + " " + fieldInfo.Name );
                }
            }
            return res;
        }
        ...
        private void WriteUnityObjectData( string depth, Object uo, bool seen )
        {
            ...
            writer.WriteLine( depth + "<unityObject type=\"{0}\" name=\"{1}\" seen=\"{2}\"/>",
                              SecurityElement.Escape( uo.GetType().Name ),
        ...
        private int ReportField(object root, string depth, FieldInfo fieldInfo)
        {
            ...
            writer.WriteLine( depth + "<field type=\"{0}\" name=\"{1}\" runtimetype=\"{2}\">",
                SecurityElement.Escape( fieldInfo.FieldType.Name ), 
                SecurityElement.Escape( fieldInfo.Name ),
                SecurityElement.Escape( v==null?"-null-":ftype.Name)
                );
                ...
                if( val != null && !seenObjects.Contains( val ))
                {
                    ...
                    else
                    {
                        ...
                        foreach( var item in val )
                        {
                            if( item != null )
                            {
                                writer.WriteLine( depth + "  <item type=\"{0}\">", SecurityElement.Escape( item.GetType().Name ) );
